...
const App: React.FC = () => {
  ...
  // --- תיקון המסך הכחול: הגדרת handleLogout בתוך ה-Scope ---
  ...
  const startConversation = async () => {
    // --- בדיקת API Key קריטית למניעת חסימה ---
    const apiKey = import.meta.env.VITE_API_KEY;
    if (!apiKey || apiKey === "undefined") {
        console.error("API KEY IS MISSING!");
        alert("שגיאה קריטית: חסר מפתח API. בדוק את ה-Environment Variables ב-Cloudflare.");
        return;
    }
    ...
    try {
      ...
      scriptProcessor.onaudioprocess = (e) => {
        if (activeSessionRef.current) {
          ...
          // --- תיקון שגיאת ה-Blob: המבנה המדויק שגוגל דורשת ---
      ...
      setStatus(ConnectionStatus.CONNECTED);
        ...
        alert("החיבור נכשל. בדוק שהמיקרופון מחובר ושיש לך הרשאות.");
    }
  };
    ...
    <div className={`h-screen bg-slate-950 flex flex-col text-slate-200 overflow-hidden font-['Inter'] ${dir}`} dir={dir}>
      <header className="p-4 flex items-center justify-between bg-slate-900/60 border-b border-white/5 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          {userData?.role === 'ADMIN' && (
            <button onClick={() => setView('ADMIN')} className="bg-white text-indigo-900 px-4 py-2 rounded-full font-black text-xs flex items-center gap-2 hover:bg-slate-200 transition-all shadow-lg animate-pulse">
              <Settings size={14} /> {t('admin_panel')}
            </button>
          )}
          <button onClick={handleLogout} className="text-xs text-slate-500 hover:text-white underline decoration-slate-700">{t('logout')}</button>
        </div>
      </header>
            ...
            <div className="grid grid-cols-2 gap-3">
              {SCENARIOS.map(s => (
                <button 
                  ...
                  className={`py-6 rounded-2xl flex flex-col items-center gap-3 transition-all ${selectedScenario.id === s.id ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(79,70,229,0.4)] scale-[1.05]' : 'bg-slate-800/40 text-slate-500 hover:bg-slate-800/70'}`}
                >
