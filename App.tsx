...
const App: React.FC = () => {
  ...
  const startConversation = async () => {
    ...
    if (!apiKey) return alert("Missing API Key. Define VITE_API_KEY in Cloudflare.");
    ...
    <div className={`h-screen bg-slate-950 flex flex-col text-slate-200 overflow-hidden font-['Inter'] ${dir}`} dir={dir}>
      <header className="p-4 flex items-center justify-between bg-slate-900/60 border-b border-white/5 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          {userData?.role === 'ADMIN' && (
            <button onClick={() => setView('ADMIN')} className="bg-white text-indigo-900 px-4 py-2 rounded-full font-black text-xs">Admin Panel</button>
          )}
        ...
        <div className="w-full md:w-[450px] flex flex-col p-4 gap-4 bg-slate-900/30 border-r border-white/5">
          <div className="bg-slate-900/90 rounded-[2rem] border border-white/10 p-5 flex flex-col gap-4 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              {SCENARIOS.map(s => (
                <button key={s.id} onClick={() => setSelectedScenario(s)} className={`py-6 rounded-2xl flex flex-col items-center gap-3 transition-all ${selectedScenario.id === s.id ? 'bg-indigo-600 text-white' : 'bg-slate-800/40 text-slate-500'}`}>
          ...
          <div className="flex flex-col items-center py-6 flex-1 justify-center relative">
            ...
            <button onClick={status === ConnectionStatus.CONNECTED ? stopConversation : startConversation} className={`mt-8 px-12 py-6 rounded-full font-black text-2xl shadow-2xl flex items-center gap-3 active:scale-95 ${status === ConnectionStatus.CONNECTED ? 'bg-red-500' : 'bg-indigo-600'}`}>
...
export default App; // השורה שהייתה חסרה וגרמה לשגיאת הבנייה
